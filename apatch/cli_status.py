...
def build_status_view(target_dir: str = ".") -> Dict[str, Any]:
    ...
    session = dto.get("session") or {}
    dto["session_phase"] = session.get("phase") or st.get("phase")
    dto["next_action"] = st.get("next_action")
    if session.get("blocker"):
        dto["blocker"] = session["blocker"]
    elif st.get("failure"):
        dto["failure"] = st.get("failure")
    sess = dto.get("active_session")
    ckpt = session.get("checkpoint") or st.get("checkpoint")
    if ckpt:
        if isinstance(sess, dict):
            sess.setdefault("checkpoint", ckpt)
        elif sess is None:
            dto["active_session"] = {
                "checkpoint": ckpt,
                "phase": dto["session_phase"],
            }
    return dto
...
def format_status_plain_lines(dto: Dict[str, Any]) -> List[str]:
    ...
    blocker = dto.get("blocker")
    if blocker:
        lines.append(
            f"Blocker: {blocker.get('error_type', '?')} → "
            f"{blocker.get('recommended_action', '?')} — {blocker.get('summary', '')}"
        )
    elif dto.get("failure"):
        fail = dto["failure"]
