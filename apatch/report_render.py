...
def _format_activity_timestamp(ts: Any) -> str:
    if ts is None or ts == "":
        return "—"
    ...
    try:
        ...
        pass
    text = str(ts).strip()
    if text.endswith("+00:00") or "T" in text:
        try:
...
def _activity_author(last: Dict[str, Any], dto: Dict[str, Any]) -> str:
    for key in ("signed_by", "key_id"):
        val = last.get(key)
        if val and not _UUID_RE.match(str(val).strip()):
            return str(val)
    anchor = (dto.get("policy") or {}).get("trust_anchor") or {}
...
def _attention_spec_ids(dto: Dict[str, Any]) -> List[str]:
    names: List[str] = []
    for spec in dto.get("specs") or []:
        ...
        if stale > 0 or not spec.get("done"):
            names.append(sid)
    return names
...
def render_md(bundle: Dict[str, Any], *, locale: str = "en") -> str:
    ...
    attention = _attention_spec_ids(dto)
    lines: List[str] = []

    if locale == "ru":
        lines.append("# Отчёт apatch\n")
        lines.append(f"Выполнено **{att}** из **{total}** требований ({sm.get('percent_complete', 0)}%).")
        if attention:
            lines.append(f"**{len(attention)}** spec(s) требуют внимания: {', '.join(attention)}.")
        if hygiene.get("status") not in (None, "ok", "clean"):
            ...
            if hygiene.get("gc_recommendation"):
                lines.append(f"Рекомендация: `{hygiene.get('gc_recommendation')}`.")
        if failure:
        ...
        if attention:
            lines.append(f"**{len(attention)}** spec(s) need attention: {', '.join(attention)}.")
        if hygiene.get("status") not in (None, "ok", "clean"):
            lines.append(f"Workspace hygiene: **{hygiene.get('status')}**.")
            if hygiene.get("gc_recommendation"):
                lines.append(f"Recommendation: `{hygiene.get('gc_recommendation')}`.")
        if failure:
    ...
    if activity:
        last = activity[0]
        author = _activity_author(last, dto)
        ts = _format_activity_timestamp(last.get("timestamp"))
        if locale == "ru":
            lines.append(f"Последнее изменение: {ts}, автор: {author}.")
        else:
            lines.append(f"Last change: {ts}, author: {author}.")

    ds = dto.get("diagnostics_summary") or {}
    if ds.get("count"):
        if locale == "ru":
            lines.append("## Проверка и диагностика")
            lines.append(
                f"Диагностик: **{ds.get('count')}**, тип: **{ds.get('top_type') or '—'}**, "
                f"сессия: `{ds.get('last_session_id') or '—'}`."
            )
        else:
            lines.append("## Verification & diagnostics")
            lines.append(
                f"Diagnostics: **{ds.get('count')}**, top type: **{ds.get('top_type') or '—'}**, "
                f"session: `{ds.get('last_session_id') or '—'}`."
            )
        if ds.get("artifact_path"):
            tail = "артефакт" if locale == "ru" else "artifact"
            lines.append(f"{tail}: `{ds.get('artifact_path')}`.")
